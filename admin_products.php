    ...
    try {
        ...
        if ($select_products->rowCount() > 0) {
            ...
            if ($insert_products) {
                ...
                exit(); // this will terminate script execution after successful insertion
            }
        }
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if ($fetch_delete_image) {
            ...
            $delete_related = $conn->prepare("DELETE FROM `wishlist` WHERE pid = ?;DELETE FROM `cart` WHERE pid = ?");
            $delete_related->execute([$delete_id, $delete_id]);
            // delete product 
      ...
      <div class="flex">
         ...
         <select name="category" class="box" required>
            <option value="" selected disabled>Select Category</option>
               ...
               <option value="marbled-soap">MARBLED SOAP</option>
