...
if(isset($_POST['update_order'])) {
   ...
   // this line will validate and sanitize
   $order_id = filter_var($order_id, FILTER_VALIDATE_INT);
   $update_payment = filter_var($update_payment, FILTER_SANITIZE_STRING);

   if($order_id === false || $update_payment === false){
      echo "invalid input";
      exit();
   }
   try{
      ...
      // this line of code will check if any rows were affected
...
if(isset($_GET['delete'])){
   ...
   try{
      ...
      if($delete_orders->rowCount() > 0){
         echo "order has been deleted!";
      }else{
         echo "no order found with ID $delete_id";
      }
